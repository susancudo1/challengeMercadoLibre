...
class MercadoLibreAppTests: XCTestCase {
    ...
    func test01SuccesSearch() {
        ...
        let worker = MercadoLibreWorker()
        
        let data = Data(self.mockProducts.utf8)
        ...
        XCTAssertEqual(response![0].id, "MLA871811974")
    }
    ...
    func test03SuccessDownloadImage() {
        ...
        var response: Data?
        let exp = self.expectation(description: "Waiting for blocking")
        worker.getImage(Product(id: UUID().uuidString, title: "Prueba", price: 29.0, thumbnail: "http://mla-s1-p.mlstatic.com/909111-MLA31239994076_062019-I.jpg", imageData: nil, condition: "nuevo", seller: Seller(seller_reputation: SellerReputation(transactions: Transactions(ratings: Ratings(positive: 0.99), completed: 35))), currency_id: "COP")) { (success, imageData) in
            successResponse = success
            response = imageData
            exp.fulfill()
        }
    ...
    func test04FailDownloadImage() {
        ...
        worker.getImage(Product(id: UUID().uuidString, title: "Prueba", price: 29.0, thumbnail: "http://mla-s1LA31239994076_06", imageData: nil, condition: "nuevo", seller: Seller(seller_reputation: SellerReputation(transactions: Transactions(ratings: Ratings(positive: 0.99), completed: 35))), currency_id: "COP")) { (success, imageData) in
            successResponse = success
            response = imageData
            exp.fulfill()
        }
